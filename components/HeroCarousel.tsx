  ...
  return (
    <section className="relative bg-primary-dark text-white h-[80vh] sm:h-[85vh] flex flex-col items-center justify-center text-center px-4 overflow-hidden">
      ...
      <div className={`relative z-10 transition-opacity duration-700 ${isAnimating ? 'opacity-0' : 'opacity-100'}`}>
        ...
        <p className="mt-4 text-xl sm:text-2xl md:text-4xl font-semibold text-accent-orange tracking-wide animate-fadeInUp" style={{ animationDelay: '0.3s' }}>
          We Measure Precise Values
        </p>
        <p className="mt-6 max-w-2xl mx-auto text-base sm:text-lg text-gray-200 animate-fadeInUp" style={{ animationDelay: '0.5s' }}>
          We provide you with precise, tech-enabled evaluations of vehicles, offering vital insights for a myriad of purposes.
        </p>
        <Link
          to="/contact"
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 flex space-x-3">
        {CAROUSEL_SLIDES.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${currentIndex === index ? 'bg-accent-orange scale-125' : 'bg-white/50 hover:bg-white'}`}
