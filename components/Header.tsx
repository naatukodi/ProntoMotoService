...
const Header: React.FC = () => {
  ...
  const getLinkClass = ({ isActive }: { isActive: boolean }) =>
    `relative px-3 py-2 rounded-md text-sm font-bold transition-colors duration-300 after:content-[''] after:absolute after:left-0 after:bottom-0 after:h-[3px] after:bg-accent-orange after:transition-all after:duration-300 hover:after:w-full text-gray-600 hover:text-accent-orange ${
      isActive ? 'after:w-full text-accent-orange' : 'after:w-0'
    }`;

  const headerClasses = `sticky top-0 z-40 ${isScrolled
    ? 'bg-white border-b border-gray-100'
    : 'bg-white'}`;
  
  const iconClasses = 'text-gray-600 hover:text-accent-orange';
  ...
  return (
    <>
              ...
              {NAV_LINKS.map((link) => (
                link.to === '/services' ? (
                  ...
                  >
                    ...
                    >
                      <div className="bg-white/95 backdrop-blur-xl rounded-lg shadow-2xl border border-gray-100 w-96 p-6">
                        <div className="space-y-4">
                          {CORE_SERVICES.map(service => (
                            ...
                            >
                              ...
                              <div>
                                <h4 className="font-bold text-gray-800 group-hover:text-accent-orange transition-colors">{service.title}</h4>
      ...
      >
        ...
        <div 
          id="mobile-menu"
          className={`fixed top-0 right-0 h-full w-full max-w-xs bg-white shadow-xl transition-transform duration-500 ease-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}
        >
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block px-4 py-3 rounded-md text-lg font-medium transition-all duration-500 ease-out ${
                      isActive ? 'bg-accent-orange text-white' : 'text-gray-600 hover:bg-gray-100 hover:text-accent-orange'
